...
class LSCricket(LSBase):
	def __init__(self):
        super().__init__()
		self.fpath=os.getcwd()
		load_dictionary(fpath+"/dicts/cricket.txt",self.dict)
		load_dictionary(fpath+"/urls/cricket.txt",self.table_urls)

	def process_query(self,query):
		html=super(LSCricket, self).fetch_page("http://www.livescore.com/cricket/")
	...
	def search_page(self,html):
		...
		leagues=soup.findAll('table',{'class':'league'})+soup.findAll('table',{'class':'league-cricket'})
		thelist=[]
